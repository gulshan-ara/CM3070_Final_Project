 ...
 */
test("fires onChangeText when text is typed", () => {
  // mocking the onChangeText function
	const onChangeText = jest.fn();
  ...
  // storing the placeholder text in a variable
	...
	fireEvent.changeText(placeHolderText, "Changed Text after completing the event");

	// Assertion of calling the mock function the typed text on text input field
	expect(onChangeText).toHaveBeenCalledWith("Changed Text after completing the event");
});
